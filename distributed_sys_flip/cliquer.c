...
static __thread int *clique_size;      /* c[i] == max. clique size in {0,1,...,i-1} */
static __thread set_t current_clique;  /* Current clique being searched. */
static __thread set_t best_clique;     /* Largest/heaviest clique found so far. */
static __thread struct tms cputimer;      /* Timer for opts->time_function() */
static __thread struct timeval realtimer; /* Timer for opts->time_function() */
static __thread int clique_list_count=0;  /* No. of cliques in opts->clique_list[] */
static __thread int weight_multiplier=1;  /* Weights multiplied by this when passing
				  * to time_function(). */

/* List cache (contains memory blocks of size g->n * sizeof(int)) */
static __thread int **temp_list=NULL;
static __thread int temp_count=0;
...
static __thread int clocks_per_sec=0;
